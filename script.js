...
const fciBreeds = Array.isArray(window.FCI_BREEDS) ? window.FCI_BREEDS : [];
...
function renderBreedCards(filter = "all") {
  ...
  const visible = fciBreeds.filter((breed) => filter === "all" || String(breed.group) === filter);

  cardsContainer.innerHTML = visible
    .map((breed) => {
      return `
      <article class="card">
        <div class="card__icon">${iconByGroup(breed.group)}</div>
        <h3>${escapeHtml(breed.name)}</h3>
        <p class="tag">FCI #${escapeHtml(breed.fciNo)} · 第${escapeHtml(breed.group)}组</p>
        <p>${escapeHtml(breed.groupName)}</p>
      </article>
    `;
    })
    .join("");
